...
public class APIPasswordTest {
    ...
    public void testChangePassword () throws Exception {
        ...
        mvc.perform( put( "/api/v1/password" ).contentType( MediaType.APPLICATION_JSON )
    ...
    public void testResetPassword () throws Exception {
        final PasswordChangeForm form = new PasswordChangeForm();
        final User patient = User.getWhere( "username='patient'" ).get( 0 );
        ...
        mvc.perform( post( "/api/v1/password/patient" ).contentType( MediaType.APPLICATION_JSON )
                .content( TestUtils.asJsonString( form ) ) ).andExpect( status().isConflict() );
        ...
        mvc.perform( post( "/api/v1/password/invalid" ).contentType( MediaType.APPLICATION_JSON )
    ...
    public void testRequestReset () throws Exception {
        ...
        // Try to request an email for a non-existent User
        mvc.perform( get( "/api/v1/password/requestReset/NULL_USER_DOESNT_EXIST" )
                .contentType( MediaType.APPLICATION_JSON ) ).andExpect( status().isNotFound() );

        // Try to request an email for a User w/o an email
        mvc.perform( get( "/api/v1/password/requestReset/hcp2" ).contentType( MediaType.APPLICATION_JSON ) )
                .andExpect( status().isBadRequest() );

        // Try to request an email for a User whose account is perma. disabled
        mvc.perform( get( "/api/v1/password/requestReset/disabled" ).contentType( MediaType.APPLICATION_JSON ) )
                .andExpect( status().isBadRequest() );

        // Try to request an email for a Patient (AliceThirteen)
        mvc.perform( get( "/api/v1/password/requestReset/AliceThirteen" ).contentType( MediaType.APPLICATION_JSON ) )
                .andExpect( status().isOk() );

        // Try to request an email for a Personnel (hcp)
        mvc.perform( get( "/api/v1/password/requestReset/hcp" ).contentType( MediaType.APPLICATION_JSON ) )
                .andExpect( status().isOk() );
    }
}
